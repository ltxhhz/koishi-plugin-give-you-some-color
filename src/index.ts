...
export function apply(ctx: Context, config: Config) {
  ...
  ctx
    ...
    .alias('颜色', '给你点颜色看看')
    .option('template', '-t 使用或不使用模板')
    .option('width', '-w <width:number> 图片宽度，默认 100')
    .option('xwidth', '-x <width:number> 图片宽度，默认 100')
    .option('height', '-y <height:number> 图片高度，默认 100')
    .option('random', '-r 随机颜色')
    ...
    .action(async ({ options }, input) => {
      ...
      if (!input) {
        if (config.random || options.random) {
      ...
      try {
        ...
        if (config.template.enable != options.template) {
          ...
          const canvas = new Canvas(options.width || options.xwidth || config.width, options.height || config.height)
          const cCtx = canvas.getContext('2d')
          cCtx.fillStyle = co.hexa()
          cCtx.fillRect(0, 0, canvas.width, canvas.height)
          return h.image(canvas.toBufferSync('png'), 'image/png')
